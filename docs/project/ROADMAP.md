...
| Library | Saved item appears; refresh, search, sort, filter, open, and delete all work | **Fail** — Pixel 9a, 2026-07-22; saved item appeared and opened correctly. Existing newest/oldest sorting reversed the entries correctly. Searching for a known distinctive word returned no result. Deleting removed the item, but did so immediately without confirmation. Refresh and filter still to test |
