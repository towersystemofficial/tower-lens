...
| Library | Saved item appears; refresh, search, sort, filter, open, and delete all work | **In progress** — Pixel 9a, updated 2026-07-25 after PR #37; existing data and Markdown, nested folders, breadcrumbs/Up navigation, file/folder deletion confirmation, and restart persistence pass. Saving into a selected custom folder was not yet available. Newest/Oldest sorting passes; Name sorting was found to use the hidden generated filename rather than a visible user-facing name. Search scope was not retested in this build. |
...
**Continue Step 1 with folder-targeted saves and editable filenames.** Let Home, ToS, and Watchlist saves select any nested Library folder and edit a generated Markdown filename; display and sort entries by that filename, sanitize invalid characters, and prevent accidental overwrites. Then complete move and rename operations in the next focused increment.
