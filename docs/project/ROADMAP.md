...
**Continue Step 1 with AI-suggested save titles.** Have the existing Home and ToS AI response generate a short descriptive title in the same request, pre-fill the editable save filename with that title, and retain the timestamp filename as a compatibility fallback. Then complete move and rename operations in the next focused increment.
