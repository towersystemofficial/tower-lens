...
7. **Prepare for public distribution.** Build the shared production backend for the active app, add accounts/authentication and Google Play Billing, and resolve broad Android storage permission before store release. Price Check deployment and activation remain excluded here and are handled under Step 10.
   ...
   **Approved beta billing model:**
   ...
   - Give no free starting credits during beta. Grant 1.5x the normal credits on the user's first completed credit purchase instead.
   ...
   - Derive credits from Sonnet's standard post-introductory output-token ceiling and gross prices up for Google Play's 15% service fee: `normal credits = floor(purchase dollars x 0.85 / $15 x 1,000,000)`. This yields 56,666 base credits per $1 after rounding down, with every larger preset and custom amount calculated from the same formula. Do not subtract Google's fee from the Claude/server/developer shares after purchase; it is already covered by the gross purchase price.
   - The 1.5x first-purchase grant intentionally converts the normal 3x usage model into an effective 2x charge (`3 / 1.5 = 2`): one share covers Claude and one covers server upkeep, leaving no developer-profit share on that purchase. Because the 15% Play fee is already grossed into the credit calculation, the bonus remains break-even under the approved two-cost-share model rather than creating a loss.
