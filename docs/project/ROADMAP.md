...
| Watchlist | Add/remove entries; matching and non-matching ingredient checks behave correctly | **Partial** — Pixel 9a, 2026-07-22; adding `peanut` succeeded, a scanned peanut bag triggered the expected warning, and an unrelated scanned item produced no warning. Removing `peanut` succeeded, but the empty Watchlist disables checking, so post-removal matching was not testable and must not be counted as a pass |
