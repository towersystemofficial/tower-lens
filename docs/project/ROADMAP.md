...
| Library | Saved item appears; refresh, search, sort, filter, open, and delete all work | **Fail** — Pixel 9a, updated 2026-07-25; saved items, refresh, opening, newest/oldest sorting, case-insensitive keyword search, structured Markdown content, and combined folder filtering/search all pass. Search correctly limits results to the selected folder. Deleting removes the item, but still does so immediately without confirmation, so this row remains failed until the safety prompt is implemented and verified. |
