...
7. **Prepare for public distribution.** First build the first-launch/setup tutorial as an independent increment. Then build the shared production backend for the active app, add Google authentication, server-authoritative balances/metering, and verified Google Play Billing purchases, and resolve broad Android storage permission before store release. Price Check deployment and activation remain excluded here and are handled under Step 10.
   ...
   **Approved beta billing model:**
   ...
   - Keep the currency legible: one estimated Claude token corresponds to one base credit. Apply the approved 3.5x usage charge: 1x toward direct AI processing costs, 1x toward server and infrastructure costs, 1x toward developer compensation and continued development, and 0.5x toward Google Play service fees. Every tool must show the 3.5x credit estimate before a run rather than exposing the unmultiplied provider-token estimate.
   ...
   - Grant a clean 50,000 normal credits per whole purchase dollar. Presets therefore grant 50,000 / 100,000 / 250,000 / 500,000 / 1,000,000 credits for $1 / $2 / $5 / $10 / $20. Custom amounts use the same 50,000-credits-per-dollar rule. This deliberately rounds down from the output-rate-derived 66,666 credits per dollar; together with the 3.5x usage charge, that conservative grant provides additional margin for provider, infrastructure, Google Play, and development costs even on the first-purchase bonus under the approved simplified cost model.
   - Grant 1.5x credits on the first completed purchase: 75,000 / 150,000 / 375,000 / 750,000 / 1,500,000 for the presets. Under the simplified share model, `3.5 / 1.5 = 2.333...x`; the conservative 50,000-credit pack grant keeps the first-purchase promotion below the normal operating multiplier while giving up part of the intended margin.
   - The estimate is not the final debit. After each successful Claude request that produces a usable result, the production backend must read Claude's reported `input_tokens` and `output_tokens`, sum them, calculate `ceil(actual provider tokens x 3.5)`, and atomically debit that many credits from the user's server-side balance. Any failed, empty, invalid, or otherwise unusable result must debit zero credits even if the AI provider reports billable token usage; Tower Systems absorbs that provider cost. The app must display the server-confirmed debit; it must never calculate or authorize the balance mutation locally.
