...
| Library | Saved item appears; refresh, search, sort, filter, open, and delete all work | **Fail** — Pixel 9a, 2026-07-22; saved item appeared and opened correctly. Existing newest/oldest sorting reversed the entries correctly. Searching for a known distinctive word returned no result. Deleting removed the item, but did so immediately without confirmation. The filter control is present, but its behavior with files inside the selected folder/type has not been exercised, so filtering remains unverified. Returning to Library after saving a new Home item displayed it automatically without restarting or manual refresh |
