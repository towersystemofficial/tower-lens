...
**Current milestone:** The core private-development app is implemented and device-verified through Step 6. Price Check's full staged code was merged in PR #59, but the feature is intentionally hidden from the launcher and deferred until beta/public release. Tower Lens now presents the four active tools followed by an always-last `More features coming soon` card. Production distribution infrastructure remains Step 8, and deeper shader/asset art direction remains Step 12.

**Next milestone:** Prepare the active app for public distribution in Step 8. Resume Price Check activation, backend deployment, live-item device verification, and tool-specific prompt refinement during the beta/public-release phase rather than treating them as the next private-alpha priority.
...
8. **Prepare for public distribution.** Build the shared production backend for the active app, add accounts/authentication as needed, credits or subscriptions and Google Play Billing, and resolve broad Android storage permission before store release. As beta/public scope is finalized, deploy and harden the preserved Price Check backend and restore its launcher entry when the feature is ready to ship.

9. **Beta testing.** Run the app with the invited beta group and record functional bugs, confusing flows, output-quality failures, cost/latency problems, and real-world OCR limits. Include hostile OCR cases such as small print, dense and multi-column pages, uneven lighting, and angled documents; add further camera processing only when repeated beta evidence justifies it. If Price Check is activated for the beta, include its live-item, citation, safety-gate, cost/latency, save/import, and prior-run comparison checks here.

10. **Second AI prompt refinement.** Use beta-test output to perform a second app-wide prompt-quality pass after every active major tool and live pipeline can be evaluated together. Revisit summaries, ToS analysis, Simplify Text, High-Fidelity OCR, Watchlist, and any other active AI-backed mode whose real-world output shows recurring quality or faithfulness problems. Include Price Check here only if it is activated during beta; otherwise perform its tool-specific pass when public-release activation resumes.
...
**Prepare the active four-tool app for public distribution.** Define and implement the shared backend/account/metering boundary, resolve the Android storage-permission release blocker, and keep Price Check dormant until beta/public activation is intentionally resumed. Shader research remains the final roadmap item and should begin only after references are collected.
